...
- Added BaseDAOHibernate with convenience methods: getObject(), saveObject,
  removeObject().
- Added "indexedProperties" attribute to @struts.form tag.  This is so indexed
  property support is only generated when the user wants it to be.  Indexed
  property support is not generated by default. Here is an example: 
