...
/**
 * This class is a base class for all appfuse plugin components to use for
...
public abstract class MojoBase extends AbstractMojo {
	...
	public void setReverseEngineeringConfigurationFile(
			String reverseEngineeringConfigurationFile) {
	...
	/**
	 * The full name of the location of the hibernate format database properties
	 * file.
	 * 
	 * @parameter expression="${appfuse.database.properties}"
	 *            default-value="${basedir}/target/classes/jdbc.properties"
	 */
	private String databasePropertiesFile;

	/**
	 * The location and name of the file that defines the attrbutes and other
	 * data that will be used to control the reverse engineering process. This
	 * file controls such things as the schema and table names that will be
	 * reverse engineered.
	 * 
	 * @parameter expression="${appfuse.reveng.file}" default-value =
	 *            "${basedir}/src/main/resources/hibernate.reveng.xml"
	 */
	private String reverseEngineeringConfigurationFile;

	/**
	 * The full package and classname for any additional
	 * ReverseEngineeringDelegator class that might be used to alter the way the
	 * reverse engineering might be handled. A sample is included in the code
	 * for this plugin and to use that class you would set this value to
	 * org.codehaus.mojo.appfuse.reveng.AppFuseReverseEngineeringDelegator.
	 * 
	 * @parameter expression="${appfuse.revstrategy.class}"
	 *            default-value="org.appfuse.mojo.appfuse.reveng.AppFuseReverseEngineeringDelegator"
	 */
	private String reverseStrategyClass;

	/**
	 * The name of the freemarker template that will be used to reverse engineer
	 * the hbm files from the database.
	 * 
	 * @parameter expression="${appfuse.hbm.template}"
	 *            default-value="hbm/hibernate-mapping.hbm.ftl"
	 */
	private String hbmTemplateName;

	/**
	 * This is the name of the mojo that is used when outputing logging
	 * information for any mojos that extend this based class.
	 */
	private String mojoName;

	/**
	 * An ant format expression defining which files you would like to process
	 * with the mojo. The pattern should not include the suffix as that will be
	 * determined by the mojo as to what type of file is to be processed. The
	 * default is to process all pojo's. A trick to allow you to dynamically
	 * define this on the command line is to assign this value in the pom to a
	 * property value such as ${appfuse.file.pattern} Then you can either define
	 * a property in a pom or settings.xml file or on the command line like
	 * -Dappfuse.file.pattern=**forwardslashUser* and this will process all
	 * objects in the that begin with User.
	 * 
	 ...
	 * 
	 * @parameter expression="${appfuse.base.package}"
	 *            default-value="org.appfuse"
	 */
	private String basePackageName;

	/**
	 * This is the location of the directory where the model hbm.xml files will
	 * be placed during the copy operation for inclusion in the project.*
	 * 
	 * @parameter expression="${appfuse.model.directory}"
	 *            default-value="${basedir}/src/main/resources"
	 */
	private String modelDirectory;

	/**
	 * This is the location of the directory where the source java files will be
	 * placed for inclusion in the project.*
	 * 
	 * @parameter expression="${appfuse.source.directory}"
	...
	/**
	 * This parameter will indicate how to manage any copy operations that take
	 * place in the mojo set. During context creation the master context files
	 * will always be updated and the existing entries replaced. When running a
	 * copy operation on any of the generated java files this flag will
	 * determine what type of copy to execute. If set to true the existing java
	 * files in the target directory will be overwritten and if set to false
	 * (default) the new files will be added to the target directory but
	 * existing files will not be overwritten by new files.
	 * 
	...
	/**
	 * A properties object passed to the exporter to be influence the object
	 * that could be used to process the data or to create objects to be used
	 * inside the templates for complex processing. In general the user should
	 * never set these properties. The design is to use default over
	 * configuration. If you accept the defaults you will generate code that is
	 * in alignment with the current appfuse standard including naming and file
	 * locations. This capability it provided for advanced developers who
	 * understand the appfuse structure and the plugin function and wish to
	 * change the default behavour of the plugin.
	 * 
	...
	/**
	 * The destination file used with config mojos. This parameter purposely has
	 * no default because it will be used with -Dparameter most commonly when
	 * adding snippets to configuration files specific to a single POJO.
	 * 
	...
	/**
	 * The parent Spring configuration file. While Spring can be configured in
	 * multiple files, this parameter is used to add configuration snippets into
	 * the configuration file. This parameter is used with the config mojos and
	 * can be overridden if needed.
	 * 
	 * @parameter expression="${appfuse.application.context.file}" default-value =
	 *            "${basedir}/src/main/webapp/WEB-INF/applicationContext.xml"
	 */
	private String applicationContextFile;

	/**
	 * The ANT LoadFile property name. This parameter is used with the config
	 * mojos and will commonly be overridden. It has no default value because it
	 * will be used with -Dparameter most commonly when adding snippets to
	 * configuration files specific to a single POJO.
	 * 
	...
	public void execute() throws MojoExecutionException {
		...
		if (getLog().isInfoEnabled()) {
			getLog().info(
					"Running the AppFuse Mojo Base without an implementation");
		}
	...
	/**
	 * This method will return the full package name to be used for generating
	 * output for the model objects.
	 * 
	...
	protected String getModelPackageName() {
		...
		if (this.getProcessingProperties() != null
			...
			modelPackageExtension = (String) this
					.getProcessingProperties()
					.get(
							org.appfuse.mojo.appfuse.utility.AppFuseProperties.MODEL_PACKAGE_EXTENSION_PROPERTY_KEY);
		}
	...
	public String toString() {
		...
		buffer.append(", ejb3 flag = ").append(ejb3);
		buffer.append(", jdk5 flag = ").append(jdk5);
		buffer.append(", processingProperties = ").append(processingProperties);
		buffer.append(", modelPackageName = ").append(
