...
[#assign pojoNameLower = pojo.shortName.substring(0,1).toLowerCase()+pojo.shortName.substring(1)]
    ...
    [#foreach field in pojo.getAllPropertiesIterator()]
        ...
        [#foreach column in field.getColumnIterator()]
            [#if !field.equals(pojo.identifierProperty) && !column.nullable && !c2h.isCollection(field) && !c2h.isManyToOne(field)]
                ...
                <setCheckbox description="set ${field.name}" name="${pojoNameLower}Form:${field.name}" value="${data.getValueForWebTest(field.value.typeName)}"/>
                [#else]
                <setInputField description="set ${field.name}" name="${pojoNameLower}Form:${field.name}" value="${data.getValueForWebTest(field.value.typeName)}"/>
                [/#if]
            [/#if]
        [/#foreach]
        [/#if]
    ...
    <target name="Delete${pojo.shortName}" description="Deletes existing ${pojo.shortName}">
        <webtest name="delete${pojo.shortName}">
            ...
            <steps>
                &login;
                <invoke description="View ${pojo.shortName} List" url="/${pojoNameLower}s.html"/>
                <clicklink label="2"/>
