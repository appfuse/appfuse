    ...
    <target name="Search${pojo.shortName}s" description="Tests search for and displaying all people">
        <webtest name="search${pojo.shortName}s">
            &config;
            <steps>
                &login;
                <invoke description="View ${pojo.shortName}s List" url="/${pojo.shortName.toLowerCase()}s.html"/>
    ...
    <target name="Save${pojo.shortName}"
        ...
        <webtest name="save${pojo.shortName}">
            ...
            <steps>
                ...
                <setinputfield description="set firstName" name="${pojo.shortName.toLowerCase()}Form:firstName" value="Canoo"/>
                <setinputfield description="set lastName" name="${pojo.shortName.toLowerCase()}Form:lastName" value="WebTest"/>
                <clickbutton label="${'$'}{button.save}" description="Click Save"/>
                <verifytitle description="Page re-appears if save successful"
                    text=".*${'$'}{${pojo.shortName.toLowerCase()}Detail.title}.*" regex="true"/>
                <verifytext description="verify success message" text="${pojo.shortName.toLowerCase()}.updated}"/>
            </steps>
        </webtest>
    </target>

    <!-- Add a new ${pojo.shortName} -->
    <target name="Add${pojo.shortName}"
        description="Adds a new ${pojo.shortName}">
        <webtest name="add${pojo.shortName}">
            &config;
            <steps>
                &login;
                <invoke description="View ${pojo.shortName} Form" url="/${pojo.shortName.toLowerCase()}Form.html"/>
                <verifytitle description="we should see the ${pojo.shortName.toLowerCase()}Detail title"
                    text=".*${'$'}{${pojo.shortName.toLowerCase()}Detail.title}.*" regex="true"/>
                <!-- enter required fields -->
                <setinputfield description="set firstName" name="${pojo.shortName.toLowerCase()}Form:firstName" value="Jack"/>
                <setinputfield description="set lastName" name="${pojo.shortName.toLowerCase()}Form:lastName" value="Raible"/>
    ...
    <target name="Delete${pojo.shortName}"
        description="Deletes existing ${pojo.shortName}">
