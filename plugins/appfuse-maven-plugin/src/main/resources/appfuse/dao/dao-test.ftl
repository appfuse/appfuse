...
<#foreach field in pojo.getAllPropertiesIterator()>
    <#foreach column in field.getColumnIterator()>
        ...
        ${pojo.shortName.toLowerCase()}Dao.save(${pojo.shortName.toLowerCase()});
        flush();

        ${pojo.shortName.toLowerCase()} = (${pojo.shortName}) ${pojo.shortName.toLowerCase()}Dao.get(${pojo.shortName.toLowerCase()}.getId());

        assertEquals("Country", ${pojo.shortName.toLowerCase()}.getFirstName());
        assertNotNull(${pojo.shortName.toLowerCase()}.getId());

        log.debug("removing ${pojo.shortName.toLowerCase()}...");

        ${pojo.shortName.toLowerCase()}Dao.remove(${pojo.shortName.toLowerCase()}.getId());
        flush();

        try {
            ${pojo.shortName.toLowerCase()}Dao.get(${pojo.shortName.toLowerCase()}.getId());
